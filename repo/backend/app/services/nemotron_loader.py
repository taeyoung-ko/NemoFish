...
def _load_pool() -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        with open(POOL_PATH, encoding="utf-8") as f:
            _POOL_CACHE = [json.loads(line) for line in f if line.strip()]
...
def sample_profiles(count: int, seed: Optional[int] = 42) -> List[Dict[str, Any]]:
    """count명 분의 '프로필 dict'를 반환.

    1순위: 로컬 사전전처리 풀(scripts/prepare_nemotron.py 로 생성) → 다운로드/네트워크 없이 즉시.
    폴백: 데이터셋 스트리밍 → row_to_profile_dict 매핑.
    """
    pool = _load_pool()
    if pool:
