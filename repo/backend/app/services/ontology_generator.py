...
class OntologyGenerator:
    ...
    def generate(
        ...
        # 调用LLM
        # 온톨로지 생성은 "문서 → JSON 스키마" 정형 작업이라 non-thinking으로 호출.
        # (thinking 켜면 대형 문서에서 max_tokens가 사고 도중 소진돼 content=None으로 잘림)
    ...
    # 传给 LLM 的文本最大长度（5만字, 약 24.5K토큰）.
    # 온톨로지는 전체 문서의 개념/타입을 봐야 스키마를 제대로 고정하므로 문서를 최대한 통째로 넣음.
    # 입력 24.5K + 출력 6K = ~30.5K < 컨텍스트 32.7K 에 맞춰 non-thinking + max_tokens=6000 사용.
    # (더 긴 문서를 쓰려면 vLLM --max-model-len 을 키워야 함)
