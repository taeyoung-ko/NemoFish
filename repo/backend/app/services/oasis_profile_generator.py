...
class OasisAgentProfile:
    ...
    # Nemotron 인구통계/스킬 — 컬럼별 개별 필드 (병합하지 않음)
...
class OasisProfileGenerator:
    ...
    def __init__(
        ...
        # Zep客户端用于检索丰富上下文 (임베딩/리랭킹 provider = 프로젝트 모드)
        ...
        if self.zep_api_key:
            try:
                self.zep_client = Zep(api_key=self.zep_api_key, providers=providers)
...
- 机构账号发言要符合其身份定位"""
    ...
    def generate_profiles_from_nemotron(
        self,
        count: int,
        ...
        for idx, d in enumerate(sample_profiles(count, seed=seed, filters=filters)):
            ...
            profile = OasisAgentProfile(
                ...
                bachelors_field=d.get("bachelors_field"),
                marital_status=d.get("marital_status"),
                military_status=d.get("military_status"),
                family_type=d.get("family_type"),
                housing_type=d.get("housing_type"),
                source_entity_uuid=d.get("source_entity_uuid"),
                source_entity_type="NemotronPersona",
            )
            profiles.append(profile)

            if progress_callback:
                progress_callback(idx + 1, count, f"Nemotron 퍼소나 {idx + 1}/{count}: {name}")
            ...
            if realtime_output_path:
                try:
                    ...
                    logger.warning(f"실시간 저장 실패: {e}")

        logger.info(f"Nemotron 퍼소나 {len(profiles)}명 생성 완료")
        return profiles
