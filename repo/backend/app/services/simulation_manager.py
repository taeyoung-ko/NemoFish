...
class SimulationManager:
    ...
    def prepare_simulation(
        ...
        try:
            ...
            # 페르소나 소스 선택: USE_NEMOTRON_PERSONAS 켜지면 Nemotron-Personas-Korea에서
            # 통계 기반 실제 한국인 표본을 샘플링(그래프 무관, LLM 생성 없음). 아니면 기존 LLM 방식.
            use_nemotron = os.environ.get("USE_NEMOTRON_PERSONAS", "").strip().lower() in ("1", "true", "yes", "on")
            if use_nemotron:
                # 우선순위: GUI에서 전달된 nemotron_count → 환경변수 → 그래프 엔티티 수 → 30
                count = nemotron_count
                if not count:
                    try:
                        count = int(os.environ.get("NEMOTRON_AGENT_COUNT", "") or total_entities or 30)
                    except (TypeError, ValueError):
                        count = total_entities or 30
                count = max(1, int(count))
                profiles = generator.generate_profiles_from_nemotron(
                    count=count,
