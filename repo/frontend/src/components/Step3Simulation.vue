      ...
      <div class="status-group">
        ...
        <div class="platform-status twitter" :class="{ active: runStatus.twitter_running, completed: runStatus.twitter_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('uiSim.platformInfoPlaza') }}</span>
          ...
          <div class="actions-tooltip">
            <div class="tooltip-title">{{ $t('uiSim.availableActions') }}</div>
        ...
        <div class="platform-status reddit" :class="{ active: runStatus.reddit_running, completed: runStatus.reddit_completed }">
          <div class="platform-header">
            ...
            <span class="platform-name">{{ $t('uiSim.platformTopicCommunity') }}</span>
              ...
              <div class="card-body">
                ...
                <template v-if="action.action_type === 'QUOTE_POST'">
                  ...
                  <div v-if="action.action_args?.original_content" class="quoted-block">
                    <div class="quote-header">
                      ...
                      <span class="quote-label">@{{ action.action_args.original_author_name || $t('uiSim.userFallback') }}</span>
                    </div>
                ...
                <template v-if="action.action_type === 'REPOST'">
                  <div class="repost-info">
                    ...
                    <span class="repost-label">{{ $t('uiSim.repostedFrom', { author: action.action_args?.original_author_name || $t('uiSim.userFallback') }) }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'LIKE_POST'">
                  <div class="like-info">
                    ...
                    <span class="like-label">{{ $t('uiSim.likedPost', { author: action.action_args?.post_author_name || $t('uiSim.userFallback') }) }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'CREATE_COMMENT'">
                  ...
                  <div v-if="action.action_args?.post_id" class="comment-context">
                    ...
                    <span>{{ $t('uiSim.replyToPost', { id: action.action_args.post_id }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'SEARCH_POSTS'">
                  <div class="search-info">
                    ...
                    <span class="search-label">{{ $t('uiSim.searchQueryLabel') }}</span>
                ...
                <template v-if="action.action_type === 'FOLLOW'">
                  <div class="follow-info">
                    ...
                    <span class="follow-label">{{ $t('uiSim.followedUser', { user: action.action_args?.target_user || action.action_args?.user_id || $t('uiSim.userFallback') }) }}</span>
                  </div>
                </template>
                ...
                <template v-if="action.action_type === 'UPVOTE_POST' || action.action_type === 'DOWNVOTE_POST'">
                  <div class="vote-info">
                    ...
                    <span class="vote-label">{{ action.action_type === 'UPVOTE_POST' ? $t('uiSim.upvotedPost') : $t('uiSim.downvotedPost') }}</span>
                  </div>
                ...
                <template v-if="action.action_type === 'DO_NOTHING'">
                  <div class="idle-info">
                    ...
                    <span class="idle-label">{{ $t('uiSim.actionSkipped') }}</span>
                  </div>
                </template>
...
const doStartSimulation = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`  ├─ ${t('uiSim.logProcessId', { pid: res.data.process_pid || '-' })}`)
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.twitter_current_round > prevTwitterRound.value) {
        addLog(t('uiSim.logPlazaRound', { round: data.twitter_current_round, total: data.total_rounds, hours: data.twitter_simulated_hours || 0, count: data.twitter_actions_count }))
        prevTwitterRound.value = data.twitter_current_round
      }
      
      if (data.reddit_current_round > prevRedditRound.value) {
        addLog(t('uiSim.logCommunityRound', { round: data.reddit_current_round, total: data.total_rounds, hours: data.reddit_simulated_hours || 0, count: data.reddit_actions_count }))
...
.actions-tooltip::before {
  ...
  border-bottom: 6px solid var(--nav-bg);
}
...
.agent-info.clickable { cursor: pointer; border-radius: var(--radius-sm); padding: 2px 4px; margin: -2px -4px; transition: background 0.15s; }
.agent-info.clickable:hover { background: rgba(0, 115, 187, 0.08); }
.agent-info.clickable:hover .agent-name { text-decoration: underline; color: var(--link-hover); }
...
/* Agent name is always the clickable trigger for the profile card;
   use link color to signal affordance even before hover. */
.agent-name {
  font-size: 13px;
  font-weight: 600;
  color: var(--link);
...
.badge-post { background: var(--surface-2); color: var(--ink); border-color: var(--border); }
.badge-comment { background: var(--surface-2); color: var(--ink-muted); border-color: var(--border); }
.badge-action { background: var(--surface-1); color: var(--ink-muted); border: 1px solid var(--border); }
.badge-meta { background: var(--surface-2); color: var(--ink-subdued); border: 1px dashed var(--border); }
...
.icon-small.filled {
  color: var(--ink-subdued); /* Keep icons neutral unless highlighted */
}
...
@keyframes ripple {
  0% { transform: scale(0.8); opacity: 1; border-color: var(--ink-subdued); }
...
.log-content::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.2); border-radius: var(--radius-sm); }
...
.log-time { color: rgba(255, 255, 255, 0.45); min-width: 75px; }
.log-msg { color: var(--nav-ink-inactive); word-break: break-all; }
