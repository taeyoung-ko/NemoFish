<template>
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        <button class="tool-btn" @click="$emit('refresh')" :disabled="loading" :title="$t('graph.refreshGraph')">
          ...
          <span class="btn-text">{{ $t('uiGraph.refresh') }}</span>
        </button>
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="selectedItem" class="detail-panel">
          <div class="detail-panel-header">
            ...
            <span v-if="selectedItem.type === 'node'" class="detail-type-badge" :style="{ background: selectedItem.color, color: 'var(--on-primary)' }">
          ...
          <div v-if="selectedItem.type === 'node'" class="detail-content">
            <div class="detail-row">
              <span class="detail-label">{{ $t('uiGraph.fieldName') }}</span>
              <span class="detail-value">{{ selectedItem.data.name }}</span>
            </div>
            <div class="detail-row">
              <span class="detail-label">{{ $t('uiGraph.fieldUuid') }}</span>
              <span class="detail-value uuid-text">{{ selectedItem.data.uuid }}</span>
            </div>
            <div class="detail-row" v-if="selectedItem.data.created_at">
              <span class="detail-label">{{ $t('uiGraph.fieldCreated') }}</span>
              <span class="detail-value">{{ formatDateTime(selectedItem.data.created_at) }}</span>
            </div>
            
            <!-- Properties -->
            <div class="detail-section" v-if="selectedItem.data.attributes && Object.keys(selectedItem.data.attributes).length > 0">
              <div class="section-title">{{ $t('uiGraph.fieldProperties') }}</div>
              <div class="properties-list">
                <div v-for="(value, key) in selectedItem.data.attributes" :key="key" class="property-item">
                  <span class="property-key">{{ key }}:</span>
                  <span class="property-value">{{ value || $t('uiGraph.valueNone') }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="detail-section" v-if="selectedItem.data.summary">
              <div class="section-title">{{ $t('uiGraph.fieldSummary') }}</div>
              <div class="summary-text">{{ selectedItem.data.summary }}</div>
            </div>
            
            <!-- Labels -->
            <div class="detail-section" v-if="selectedItem.data.labels && selectedItem.data.labels.length > 0">
              <div class="section-title">{{ $t('uiGraph.fieldLabels') }}</div>
          ...
          <div v-else class="detail-content">
            ...
            <template v-if="selectedItem.data.isSelfLoopGroup">
              <div class="edge-relation-header self-loop-header">
                {{ selectedItem.data.source_name }} - {{ $t('uiGraph.selfRelations') }}
                <span class="self-loop-count">{{ $t('uiGraph.itemsCount', { count: selectedItem.data.selfLoopCount }) }}</span>
              </div>
                ...
                >
                  ...
                  <div class="self-loop-item-content" v-show="expandedSelfLoops.has(loop.uuid || idx)">
                    ...
                    <div class="detail-row" v-if="loop.fact">
                      <span class="detail-label">{{ $t('uiGraph.fieldFact') }}</span>
                      <span class="detail-value fact-text">{{ loop.fact }}</span>
                    </div>
                    <div class="detail-row" v-if="loop.fact_type">
                      <span class="detail-label">{{ $t('uiGraph.fieldType') }}</span>
                    ...
                    <div v-if="loop.episodes && loop.episodes.length > 0" class="self-loop-episodes">
                      <span class="detail-label">{{ $t('uiGraph.fieldEpisodes') }}</span>
            ...
            <template v-else>
              ...
              <div class="detail-row">
                <span class="detail-label">{{ $t('uiGraph.fieldLabel') }}</span>
                <span class="detail-value">{{ selectedItem.data.name || 'RELATED_TO' }}</span>
              </div>
              <div class="detail-row">
                <span class="detail-label">{{ $t('uiGraph.fieldType') }}</span>
                <span class="detail-value">{{ selectedItem.data.fact_type || $t('uiGraph.valueUnknown') }}</span>
              </div>
              ...
              <div class="detail-section" v-if="selectedItem.data.episodes && selectedItem.data.episodes.length > 0">
                <div class="section-title">{{ $t('uiGraph.fieldEpisodes') }}</div>
              ...
              <div class="detail-row" v-if="selectedItem.data.valid_at">
                <span class="detail-label">{{ $t('uiGraph.fieldValidFrom') }}</span>
    ...
    <div v-if="graphData && entityTypes.length" class="graph-legend">
      <span class="legend-title">{{ $t('uiGraph.entityTypesTitle') }}</span>
    ...
    <div v-if="graphData" class="edge-labels-toggle">
      ...
      <span class="toggle-label">{{ $t('uiGraph.showEdgeLabels') }}</span>
    </div>
  </div>
</template>
...
const entityTypes = computed(() => {
  ...
  const typeMap = {}
  // Lovable 팔레트 기반 조색(차콜/어스톤 뮤트) — 크림 배경과 조화, 채도 낮춤
  const colors = ['#1C1C1C', '#4A6B82', '#4A7C59', '#A8443A', '#9C7A32', '#5C7E96', '#5F5F5D', '#8A6D3B', '#3B5468', '#93A1A8']
...
const renderGraph = () => {
  ...
  const getColor = (type) => colorMap[type] || '#879596'
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // 重置之前选中边的样式
      linkGroup.selectAll('path').attr('stroke', '#879596').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#545B64')
      // 高亮当前选中的边
      d3.select(event.target).attr('stroke', '#1C1C1C').attr('stroke-width', 3)
  ...
  const linkLabelBg = linkGroup.selectAll('rect')
    ...
    .on('click', (event, d) => {
      ...
      linkLabels.attr('fill', '#545B64')
      // 高亮对应的边
      link.filter(l => l === d).attr('stroke', '#1C1C1C').attr('stroke-width', 3)
  ...
  const linkLabels = linkGroup.selectAll('text')
    ...
    .on('click', (event, d) => {
      ...
      // 高亮对应的边
      link.filter(l => l === d).attr('stroke', '#1C1C1C').attr('stroke-width', 3)
      d3.select(event.target).attr('fill', '#1C1C1C')
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .on('click', (event, d) => {
      ...
      linkGroup.selectAll('path').attr('stroke', '#879596').attr('stroke-width', 1.5)
      // 高亮选中节点
      d3.select(event.target).attr('stroke', '#1C1C1C').attr('stroke-width', 4)
      // 高亮与此节点相连的边
    ...
    .on('mouseenter', (event, d) => {
      if (!selectedItem.value || selectedItem.value.data?.uuid !== d.rawData.uuid) {
        d3.select(event.target).attr('stroke', '#16191F').attr('stroke-width', 3)
      }
    })
...
.graph-panel {
  ...
  width: 100%;
  height: 100%;
  background-color: var(--canvas-subdued);
...
@keyframes breathe {
  0%, 100% { opacity: 0.7; transform: scale(1); filter: drop-shadow(0 0 2px rgba(29, 131, 72, 0.3)); }
  50% { opacity: 1; transform: scale(1.15); filter: drop-shadow(0 0 8px rgba(29, 131, 72, 0.6)); }
}
